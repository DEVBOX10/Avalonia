...
{
    public class CompiledBindingExpressionTests_AttachedProperty
    {
        [Fact]
        public async Task Should_Get_Attached_Property_Value()
        {
            var data = new Class1();
            var target = CompiledBindingExpression.OneWay(data, o => o[Owner.FooProperty]);
        ...
        public async Task Should_Get_Chained_Attached_Property_Value()
        {
            ...
            var target = CompiledBindingExpression.OneWay(data, o => o.Next[Owner.FooProperty]);
            var result = await target.Take(1);
        ...
        {
            Func<Tuple<CompiledBindingExpression<Class1, object>, WeakReference>> run = () =>
            {
                var source = new Class1();
                var target = CompiledBindingExpression.OneWay(source, o => o.Next[Owner.FooProperty]);
