...
{
    public class CompiledBindingExpressionTests_Converter
    {
        [Fact]
        public async Task Should_Convert_Simple_Property_Value()
        {
            var data = new { Foo = "123" };
            var target = CompiledBindingExpression.OneWay(data, o => o.Foo, x => int.Parse(x));
        ...
        public async Task Should_Convert_Simple_Property_Chain()
        {
            ...
            var target = CompiledBindingExpression.OneWay(data, o => o.Foo.Bar.Baz, x => int.Parse(x));
            var result = await target.Take(1);
        ...
        public void Should_Convert_TwoWay()
        {
            ...
            var target = CompiledBindingExpression.TwoWay(
                data,
                o => o.Foo,
                (o, v) => o.Foo = v,
                x => int.Parse(x),
